...
#define heightDiff(parent) ( ( ((parent)->right == NULL) ? 0 : (int)((parent)->right->height) ) - ( ((parent)->left == NULL) ? 0 : (int)((parent)->left->height) ) )
    ...
    #warning Value type VAL_T not defined
#endif
...
#define BLANK_AVL_TREE { NULL, 0, 0 }

AVLTree build_AVLTree() {
    AVLTree t = BLANK_AVL_TREE;
    return t;
}
...
/*
 * Returns EXIT_FAILURE (1) if the string exists,
 * else EXIT_SUCCESS (0)
 */
int avl_tree_add(AVLTree * tree, KEY_T key, VAL_T value) {
...
AVLTreeNode * avl_tree_getNode(AVLTree * tree, KEY_T key) {
    ...
    if (ptr != NULL && __compKeys(ptr->key, key) == 0) {
        return ptr;
    }
    return NULL;
}
